(function($){
	...
	$(document).ready(function(){
		...
		btn_test_settings.click(function(){
			...
			if( inp_swp_key.length && inp_swp_endpoint.length ){
				...
				.done(function( r ) {
					...
					swp_status_field.val( r.status.toString() );
				})
				.fail(function( e ) {
					console.error(e);
				});
		...
		generator.listen( generator.EVENTS.CHANGE , function( new_shortcode  ) {

			// Get argument list from Generator 	
			generator.load_api_result(preview_box);
			generator.load_api_filters(filter_box, false, function(){
				bind_save_button();
			});
		});
